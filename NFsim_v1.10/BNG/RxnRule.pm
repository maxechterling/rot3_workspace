...
##-----------------------##
{
	...
	# $rule->applyRule(SpeciesList, RxnList, (Species))
	sub applyRule
	{
		...
		if ( $pattDepth == -1 )
		{
			$rr    = shift;    #reaction rule
			$slist = shift;    #species list
			$rlist = shift;    #reaction list (to be filled?)
			$plist = shift;
			...
			foreach my $patt ( @{$rr->Reactants} )
			{
                ...
                # IN_PROGRESS: replacing symmetry handler with canonical reaction labeling
                # ..this means we don't need to filter matches based on context multiplicity
				...
				$new_matches_found +=
				  $n_new;    # cumulative count of new matches to all patterns
	  ...
	  MATCH:
		foreach my $match ( @{$rmatches->[$pattDepth]} )
		{
            ...
            # we should have a full reactant set here.
            # check that reactants form an interacting Set (compartment check)
            # this is a fixed and simplified replacement for the old compartment compatibility check.
            # --justin
			if ( @targets > 1 ) {
				next MATCH
				  unless (
					$targets[0]->interactingSet( @targets[ 1 .. $#targets ] ) );
			}
		    ...
		    # NOTE: this is required when the same species can match more than one
		    #  reactant pattern. --Justin
			my $reactant_permutations = $rr->permute_identical_reactants( \@targets );

			## Apply rule transformations and calculate labels.
			#print STDERR "\nHERE : apply operations and calculate labels\n";
			my $products = undef;
			my $rxn_labels = [];
			foreach my $permute (@$reactant_permutations)
			{
			    my ( $products_perm, $rxn_label) = $rr->apply_operations( \@matches, $permute );
			    $products = $products_perm  unless ( defined $products );
			    # if undefined products are returned, it probably means we violated the "+" or "." operator
			    next MATCH unless (defined $products);
                push @$rxn_labels, $rxn_label;
			}
            
            
            # check if this reaction is redundant.
            #   do it now to save unnecessary effort refining and labeling the products
            #print STDERR "HERE : check for redundant reactions\n";
			if ( exists $rr->RxnLabels->{ $rxn_labels->[0] } )
			{   # redundant reaction!
				next MATCH;
			}
		    ...
		    for ( my $ip = 0; $ip < @$products; ++$ip )
			{
                ...
                #  components have been lumped into the same graph.  So at this point we want to check for
                #  connectivity.
	            ...
	            #if ( $p->toString =~ /Lig\S+Lyn\S+Lyn\S+Rec\S+Rec/  and  @{$p->Molecules} == 5 )
	            #{
	            #    print "before: ", $p->toString(), "\n";
	            #}
	            ...
	            #if ( $p->toString =~ /Lig\S+Lyn\S+Lyn\S+Rec\S+Rec/  and  @{$p->Molecules} == 5 )
	            #{
	            #    print "after: ", $p->toString(), "\n";
	            #}

                #print STDERR "product species: ", $p->toString(), "\n";
                #print STDERR "label exact:     ", $p->StringExact, "\n";
                #print STDERR "label ID:        ", $p->StringID, "\n";
			...
			# calculate correction factor for identical reactants.
			my $reactant_symmetry = 1;
			{
				my %reactant_hash = ();
				foreach my $reactant (@reactant_species)
				{
					$reactant_hash{ $reactant->Index }++;
				}

				foreach my $reactant ( keys %reactant_hash )
				{
					$reactant_symmetry /= factorial( $reactant_hash{$reactant} );
				}
			}

			# print STDERR ">>> SYM: $reactant_symmetry\n";
			
		  REACTANT_PERMUTATION:
			foreach my $rxn_label (@$rxn_labels)
			{
				#print STDERR $rxn_label, "\n";

				# check if this reaction is redundant.
				if ( exists $rr->RxnLabels->{$rxn_label} )
				{   # redundant reaction!
					next REACTANT_PERMUTATION;
				}

                # register the canonical reaction label to prevent redundancy.
                #   we only get here if this reaction label is new, so no check is necessary.
				...
				$rxn->RxnRule( $rr );
				$rxn->StatFactor( $reactant_symmetry) ;
				$rxn->Compartments( $compartments );

				#print $rxn->toString( $plist ), "\n\n";
				...
				#for my $prod (@prods)
				#{
				#    print " + " unless $iprod==0;
				#    print $prod->SpeciesGraph->toString();
				#    ++$iprod;
				#}
				#print "\n";

				#for my $i (0..$pattDepth)
				#{
				#    print " $i $offsets[$i] ",$matches[$i]->Target->toString();
				#}

			}    # end REACTANT_PERMUTATION loop
...
# @permute is an array that defines a permutation of the reactants
{
	...
	# print STDERR ": reactants ordered..done :";
	...
	#print STDERR ": molecule additions ordered..done :";

	#print STDERR "num molecules: ", scalar @{$g->Molecules}, "\n";
	#foreach my $mol ( @{$g->Molecules} )
	#{
	#    print STDERR ref $mol, "\n";
	#    print STDERR $mol->Name, "\n";
	#}
	...
	#print STDERR ": state changes ordered..done :";
	...
	#print STDERR ": compartment changes ordered..done :";
	...
	#print STDERR ": edge deletions ordered..done :";
	...
	#print STDERR ": edge additions ordered..done :";
	...
	#print STDERR ": molecule deletions ordered..done :";
