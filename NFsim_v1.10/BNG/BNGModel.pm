		...
		while (1) {
			...
			while ( $_ = get_line() ) {
				...
				if (s/^\s*begin\s*//) {
					...
					elsif ( $name =~ /^[Mm]olecule[_ ][Tt]ypes$/ )
					{
						if ( !$in_model ) {
							$err = errgen(
								"$name cannot be defined outside of a model");
					...
					elsif ( $name =~ /^[Pp]opulation[_ ][Tt]ypes$/ )
					{
                        ...
                        $model->PopulationTypesList->StrictTyping(1);  # enable strict typing
					    foreach my $line ( @{$block_dat} )
						{
							my ( $entry, $lno ) = @{$line};
						    ...
						    or ( $name =~ /^seed[ _]species$/ ) )
					{
						...
						{
							my ( $entry, $lno ) = @{$line};
							if ( $err = $model->SpeciesList->readString( $entry, $model->ParamList,
...
sub writeSimpleBNGL
{
	...
	# Species
	$out .=
	  $model->SpeciesList->writeBNGL( $model->Concentrations, $model->ParamList );
...
sub simulate_ode
{
    ...
    my $continue = ( exists $params->{continue} and defined $params->{continue} ) ? $params->{continue} : 0;
    # print number of active species
    my $print_n_species_active = ( exists $params->{print_n_species_active} and defined $params->{print_n_species_active} )
                                 ? $params->{print_n_species_active} : 0;
...
sub simulate_pla
{
	...
	$command .= " -p pla";
    if (exists $params->{sim_config}){ # PLA configuration (string)
    	$command .= " \"$params->{sim_config}\"";
    }
    if (exists $params->{seed}){
    	$command .= " -h $params->{seed}";
    }
...
#	}
	...
	elsif ( defined( $params->{sample_times} ) ) { # Not implemented yet, but will be caught by Network3
...
##		# species at the edge of the network is newly populated
		if ( $message =~ s/^edgepop:\s*// ){ # NOT IMPLEMENTED YET
		}
		else {
			print $message;
			$last = $message;
...
sub simulate_ssa
{
	...
	my $print_end = ( defined( $params->{print_end} ) ) ? $params->{print_end} : 0;
	my $print_net = ( defined( $params->{print_net} ) ) ? $params->{print_net} : 0;
#	my $seed = ( defined( $params->{seed} ) ) ? $params->{seed} : int( rand( 2**32 ) ) + 1;
	my $seed = ( defined( $params->{seed} ) ) ? $params->{seed} : int( rand( 0.5*(2**32)-1 ) );
#	my $verbose = ( defined( $params->{verbose} ) ) ? $params->{verbose} : 0;
    # Added explicit argument for simulation continuation.  --Justin
    my $continue = ( exists $params->{continue} and defined $params->{continue} ) ? $params->{continue} : 0;	
	...
	my $update_interval = ( defined( $params->{update_interval} ) ) ? $params->{update_interval} : 1;
	my $expand = ( defined( $params->{expand} ) ) ? $params{$expand} : "lazy";
	if ( $expand eq "lazy" ) { }
	...
	# Print final netfile
	if ( $model->SpeciesList && ( $err = $model->writeNET( { prefix => "$netpre" } ) ) ){
		return ($err);
	}
