...
sub readString
{
    ...
    if ( $CountAutos )
    {
        @autos = $sg->isomorphicToSubgraph($sg);
...
sub splitConnectedComponents
{
    ...
    my $map_imol_to_ipatt = (@_) ? shift : undef;
    ...
    if ( defined $map_imol_to_ipatt )
    {
        ...
        $n_patt = max(@$map_imol_to_ipatt) + 1;
    }
    else
    ...
    # initialize vector of species graphs
  ...
  MITER:
	while (1)
	{
		...
		for ( $im2 = $mptr[$im1] ; $im2 <= $nmol ; ++$im2 )
		{
			...
			next if ( $molecules1[$im1]->Compartment != $molecules2[$im2]->Compartment );
...
sub cmp_molecule
{
	...
	*comp_b = $b->Components;
	...
	if ( $a->Compartment )
	{
		return ( 1 )  unless ( $b->Compartment );
		if ( $cmp = ( $a->Compartment->Name cmp $b->Compartment->Name ) )
		{   return $cmp;   }
	}
	if ( $b->Compartment )
	{
		return ( -1 ) unless ( $a->Compartment );
	}

    # compare by components
	foreach my $i ( 0 .. $#comp_a )
	{
