...

{
  ...
  sub BNGroot {
    if (!$BNG_ROOT){
      # Determine location of bionetgen executable Setting the enivroment variable
      # BioNetGenRoot can be used to override the default location of library files.
      if (defined $ENV{BioNetGenRoot}){
	# Use environment variable
	$BNG_ROOT= "$ENV{BioNetGenRoot}";
	# remove trailing slash
	$BNG_ROOT=~ s/\/$//;
	$BNG_ROOT=~ s/\\$//;
      } else {
	# Find path to BioNetGenRoot based on location of this script,
	# assuming it is in $BioNetGenRoot/Perl2
	my @dirs= split /[\\\/]/, $0;
	my $execname= pop @dirs;
	my $perldir= pop @dirs;
	if (@dirs){
	  $BNG_ROOT= join('/', @dirs);
	} else {
  ...
  sub BNGpath{
    ...
    my $path= $BNG_ROOT;
    # Determine whether sep char is back or forward slash
    if ($path=~ /\\/){
      $sep= '\\';
    } else {
      $sep= '/';
